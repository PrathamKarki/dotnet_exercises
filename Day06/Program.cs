...
class Task : Entity, IComparable<Task>
{
    ...
    public static void SyncNextId(List<Task> existingTasks)
    {
        if (existingTasks != null && existingTasks.Count > 0)
        {
            nextId = existingTasks.Max(t => t.Id) + 1;
        }
    }
...
class Program
{
    ...
    private static readonly string FilePath = @"C:\Users\prkarki\Desktop\dotnet_exercise\Day06\tasks.json";
    ...
    static void ListTasks()
    {
        ...
        foreach (Task task in tasks)
        {
            ...
            Console.WriteLine("Task tag: " + dataStatusTag);
    ...
    static void SaveTasksToFile()
    {
        try
        {
            string jsonString = JsonSerializer.Serialize(tasks, JsonOpts);
            File.WriteAllText(FilePath, jsonString);
            Console.WriteLine("Tasks written to the tasks.json successfully");
        }
        catch (Exception ex)
        {
            {
                Console.WriteLine("Error saving task to JSON: " + ex.Message);
            }
        }
    }
    ...
    static void LoadTasksFromFile()
    {
        try
        {
            if (File.Exists(FilePath))
            {
                ...
                var loaded = JsonSerializer.Deserialize<List<Task>>(jsonString);
                if (loaded != null)
                {
                    tasks = loaded;
                    Task.SyncNextId(tasks);
                    Console.WriteLine("Loaded " + tasks.Count + " task from" + FilePath);
                }
                else
                {
                    Console.WriteLine("tasks.json not found, Starting with empty task list.");
                }
            }
        }
        catch (Exception e)
        {
            Console.WriteLine("Error loading thetask from json" + e.Message);
        }
    }
